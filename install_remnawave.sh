...
start_panel_node() {
    ...
    if docker ps -q --filter "ancestor=remnawave/backend:1.6.18" | grep -q . || docker ps -q --filter "ancestor=remnawave/node:2.1.7" | grep -q .; then
...
services:
  ...
  remnawave:
    image: remnawave/backend:1.6.18
