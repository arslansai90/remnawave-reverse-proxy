...
start_panel_node() {
    ...
    if docker ps -q --filter "ancestor=remnawave/backend:2.2.6" | grep -q . || docker ps -q --filter "ancestor=remnawave/node:2.2.6" | grep -q . || docker ps -q --filter "ancestor=remnawave/backend:2" | grep -q .; then
